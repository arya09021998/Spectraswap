...
// File: contracts/SpectraDexLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title SpectraDex Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract SpectraDexLottery is ReentrancyGuard, ISpectraDexLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSpc = 50 * (uint256(10) ** 9);
    uint256 public minPriceTicketInSpc = 10 * (uint256(10) ** 9);
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSpc;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] spcPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSpc;
        uint32 finalNumber;
    }
     ...
     * @param _spcTokenAddress: address of the SPC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _spcTokenAddress, address _randomGeneratorAddress) {
        spcToken = IERC20(_spcTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SPC to this contract
        uint256 amountSpcToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSpc,
            _ticketNumbers.length
        );

        // Transfer spc tokens to this contract
        spcToken.safeTransferFrom(address(msg.sender), address(this), amountSpcToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSpc += amountSpcToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSpcToTransfer
        uint256 rewardInSpcToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSpcToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        spcToken.safeTransfer(msg.sender, rewardInSpcToTransfer);

        emit TicketsClaim(msg.sender, rewardInSpcToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSpc) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SPC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].spcPerBracket[j] =
                ...
                // A. No SPC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].spcPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInSpc - amountToShareToWinners);

        // Transfer SPC to treasury address
        spcToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        spcToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSpc += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSpc >= minPriceTicketInSpc) && (_priceTicketInSpc <= maxPriceTicketInSpc),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSpc: _priceTicketInSpc,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            spcPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSpc: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(spcToken), "Cannot be SPC token");
    ...
    /**
     * @notice Set SPC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSpc: minimum price of a ticket in SPC
     * @param _maxPriceTicketInSpc: maximum price of a ticket in SPC
     */
    function setMinAndMaxTicketPriceInSpc(uint256 _minPriceTicketInSpc, uint256 _maxPriceTicketInSpc)
        external
        onlyOwner
    {
        require(_minPriceTicketInSpc <= _maxPriceTicketInSpc, "minPrice must be < maxPrice");

        minPriceTicketInSpc = _minPriceTicketInSpc;
        maxPriceTicketInSpc = _maxPriceTicketInSpc;
    }
     ...
     * @param _priceTicket price of a ticket (in SPC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].spcPerBracket[_bracket];
        } else {
            return 0;
        }
    }
